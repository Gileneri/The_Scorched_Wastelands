...
public class ToxicwastebarrelBlock extends TheScorchedWastelandsModElements.ModElement {
	@ObjectHolder("the_scorched_wastelands:toxicwastebarrel")
	public static final Block block = null;

	public ToxicwastebarrelBlock(TheScorchedWastelandsModElements instance) {
		super(instance, 142);
	}
	...
	public static class CustomBlock extends Block implements IWaterLoggable {
		...
		public CustomBlock() {
			super(Block.Properties.create(Material.IRON).sound(SoundType.ANVIL).hardnessAndResistance(1f, 7f).setLightLevel(s -> 1).harvestLevel(2)
					.harvestTool(ToolType.PICKAXE).setRequiresTool().notSolid().setOpaque((bs, br, bp) -> false));
			this.setDefaultState(this.stateContainer.getBaseState().with(AXIS, Direction.Axis.Y).with(WATERLOGGED, false));
			setRegistryName("toxicwastebarrel");
		}
		...
		public VoxelShape getShape(BlockState state, IBlockReader world, BlockPos pos, ISelectionContext context) {
			...
			switch ((Direction.Axis) state.get(AXIS)) {
				case X :
					return VoxelShapes.or(makeCuboidShape(0, 0, 0, 23, 16, 16))
				...
				default :
					return VoxelShapes.or(makeCuboidShape(0, 0, 0, 16, 23, 16))
							...
							.withOffset(offset.x, offset.y, offset.z);
				case Z :
					return VoxelShapes.or(makeCuboidShape(0, 0, 16, 16, 16, -7))
